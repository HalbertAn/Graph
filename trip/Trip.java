...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 1; i < segment.size(); i++) {
            int desti = segment.get(i);
            path = _map.getLabel(cur, desti);
            Direction direc = path.direction();
            String road = path.toString();
            double le = path.length();
            while (i + 1 < segment.size()
                && road.equals(_map.getLabel(segment.get(i),
                segment.get(i + 1)).toString())
                && direc.equals(_map.getLabel(segment.get(i),
                    segment.get(i + 1)).direction())) {
                le += _map.getLabel(segment.get(i),
                        segment.get(i + 1)).length();
                i++;
            }
            if (i == segment.size() - 1) {
                System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                        seq, road, direc.fullName(),
                        le, _map.getLabel(segment.get(i)).toString());
                seq++;
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.%n",
                        seq, road, direc.fullName(), le);
                cur = segment.get(i);
                seq++;
            }
        }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double estimatedDistance(int v) {
            if (_map.contains(v)) {
                Location next = _map.getLabel(v);
                return _finalLocation.dist(_finalLocation, next);
            }
            return 0.0;
        }
