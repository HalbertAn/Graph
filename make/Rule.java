...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new Error("Error: command is not empty");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int x : _depends.successors(getVertex())) {
            Rule temp = _depends.getLabel(x);
            if (temp.isUnfinished()) {
    ...
    private boolean outOfDate() {
        if (getTime() == null) {
            ...
            for (Integer x : _depends.successors(this.getVertex())) {
                Rule temp = _depends.getLabel(x);
                if (this.getTime() < temp.getTime() || temp.getTime() == null) {
                    return true;
                }
            }
            return false;
